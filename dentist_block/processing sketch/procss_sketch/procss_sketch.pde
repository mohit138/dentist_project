...
//___ THIS SKETCH TAKES CONSTANT TEMPERATUR AND BUTTON VALUES FROM ARDUINO SERIAL PORT AND DATABASE. 
// AND USES THESE VALUES TO UPDATE TEMPERATURES IN DATABASE. 
// THE VARIABLES USED ARE FOR THE PURPOSE OF DETECTING SINGLE BUTTON PRESSES
...
void setup() {
  String user     = "root";
  String pass     = "";
  String database = "dentist_db"; // enter the name of your database
  
  msql = new MySQL( this, "localhost", database, user, pass );
    
  // IF YOU GET ARRAY OUT OF INDEX ERROR FROM HERE, CEHCK COM PORT VALUE, ACCORDING TO ARDUINO IDE. ie MIGHT NEED TO CHANGE[--] VALUE!!!
  ...
  if ( msql.connect() )
    {
      ...
      if(flag==0 && side<128)
      {
        ...
        //check if button is also pressed
        if(int(a[0]) == 1)
        {
